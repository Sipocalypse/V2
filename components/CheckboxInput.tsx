...
const CheckboxInput: React.FC<CheckboxInputProps> = ({ id, label, checked, onChange, disabled }) => {
  return (
    <div className="flex items-center">
      ...
      />
      <label htmlFor={id} className={`ml-2 text-sm ${disabled ? 'text-gray-500' : 'text-gray-200'}`}>
        {label}
      </label>
    </div>
  );
