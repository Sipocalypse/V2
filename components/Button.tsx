...
const Button: React.FC<ButtonProps> = ({ 
  ...
  const variantStyles = {
    primary: "bg-custom-lime hover:bg-lime-400 text-custom-pink font-comic shadow-md hover:shadow-lg focus:ring-custom-pink disabled:opacity-60 disabled:cursor-not-allowed", // Changed text color
  ...
  let actualSpinnerColor = 'text-white'; // Default for secondary
  if (variant === 'primary') {
    actualSpinnerColor = 'text-custom-pink'; // Changed spinner color
  ...
  return (
    <button 
      ...
      {...props}
    >
      {isLoading && <LoadingSpinner size="sm" className="mr-2" color={actualSpinnerColor} />}
      {children}
    </button>
  );
};
